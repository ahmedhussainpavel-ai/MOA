...
export const Input: React.FC<InputProps> = ({ label, className = '', ...props }) => (
  <div className="flex flex-col gap-1 w-full">
    ...
    <input 
      className={`bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-moa-cream focus:outline-none focus:border-moa-gold focus:ring-1 focus:ring-moa-gold transition-colors ${className}`}
      {...props}
    />
  </div>
);
...
export const Card: React.FC<HTMLMotionProps<"div">> = ({ children, className = '', ...props }) => (
  <motion.div 
    className={`bg-white/5 backdrop-blur-md border border-white/5 rounded-2xl p-5 shadow-xl ${className}`}
...
export const Badge: React.FC<{ children: React.ReactNode; variant?: 'success' | 'warning' | 'info' | 'danger' }> = ({ children, variant = 'info' }) => {
  ...
  return (
    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${colors[variant]}`}>
      {children}
    </span>
  );
};
      ...
      <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
          <motion.div 
            ...
            className="bg-[#1C1C1C] border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl pointer-events-auto flex flex-col max-h-[90vh]"
          >
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              <h3 className="font-display font-bold text-xl text-moa-gold">{title}</h3>
              <button onClick={onClose} className="p-1 hover:bg-white/10 rounded-full transition-colors text-moa-cream/60 hover:text-white">
                <X size={20} />
              </button>
            </div>
  ...
  return (
    <div className="w-full">
      <div 
        ...
        onDrop={onDrop}
        className={`relative h-48 rounded-xl border-2 border-dashed transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center gap-2 group
